...
import { copyCourse } from "../Requests/copyRequests"
...
class MyForm extends React.Component {
    ...
    handlePressCopy = async (cid) => {
      let message = "Are you sure you want to copy this course template into your Canvas course?"
      if (window.confirm(message)) {
        let res = await copyCourse(this.props.userInfo.courseId, cid, this.props.token)

        if (res.success) {
          window.alert("Success: Course content import is in progess"
          +  "\n" + "You'll be able to see your new content shortly."
          +  "\n" + CANVAS_URL + "courses/cid/content_migrations")
        } else {
          window.alert("Error: There was a problem importing this course's content")
        }
      }
    }
