      ...
      return (
        <div style={{ fontFamily: 'arial, sans-serif', width: '100%', height: '100%', margin: 'auto', border: '1px solid white', borderRadius: '5px', padding: '8px', display: 'flex', flexDirection: 'column'}}>
         ...
         <Table striped bordered style={{color: 'whitesmoke'}}>
             <caption style={{ captionSide: 'top', color: 'whitesmoke', textAlign: 'center'}}>Student Log Report for {this.props.courseInfo.short_name}</caption>
             <thead>
                 <th>#</th>
                 <th>Name</th>
                 <th>Elapsed Time</th>
             ...
             <tbody>
                {this.props.logReport.map((e, i) => {
                    return (<tr>
                        <td>{(i+1)}</td>
                        <td><a href={'https://montclair.instructure.com/courses/' + this.props.courseInfo.courseId + '/users/' + e.id}>{e.username}</a></td>
