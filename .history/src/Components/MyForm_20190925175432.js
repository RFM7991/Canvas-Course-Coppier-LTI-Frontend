...
class MyForm extends React.Component {
   ...
   async handleSubmit() {
      if (this.state.actionTaken) {
        ...
        let response  = await updateCourse(this.props.loginInfo.course_id, this.state.checked)
        .then(res => {
          alert(response)
          this.setState({ actionTaken : false })
        })
        .catch(err => alert('Sorry, your request could not be completed at this time. Please contact..Rob I guess. Error:', err))
