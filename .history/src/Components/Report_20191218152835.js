    ...
    render() {
      ...
      return (
        <div style={{ fontFamily: 'arial, sans-serif', width: '100%',  border: '1px solid white', borderRadius: '5px', padding: '8px', display: 'flex', flexDirection: 'column'}}>
          <div style={{ width: '100%', marginBottom: '5px', display: 'flex', justifyContent: 'spae-between'}}>
          <div style={{}}><span>Spacer</span></div>
            <span style={{ alignSelf:'center', color: 'whitesmoke', fontWeight: 'bold'}}>Student Log Report for {this.props.courseInfo.short_name}</span>
         ...
         <Table variant="dark" striped bordered>
             ...
             <tbody>
                {this.props.logReport.map((e, i) => {
                    return (<tr key={i}>
                        <td>{(i+1)}</td>
                        <td><a target="_blank" href={'https://montclair.instructure.com/courses/' + this.props.userInfo.courseId + '/users/' + e.user_id}>{e.username}</a></td>
